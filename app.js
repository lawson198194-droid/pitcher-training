...
let spots = [];
let badSpots = [];
let undoStack = [];  // Undo stack: stores last action for undo
...
canvas.addEventListener('click', (e) => {
    ...
    if (isInStrikeZone) {
        spots.push({ x, y });
        // Save to undo stack
        undoStack.push({ type: 'strike', x, y });
        ...
        badSpots.push({ x, y });
        // Save to undo stack
        undoStack.push({ type: 'ball', x, y });
        // Balls not auto-incremented, manual record only
    }
...
function setupEventListeners() {
    ...
    document.getElementById('clearSpots').addEventListener('click', () => {
        ...
        undoStack.push({ type: 'clear', spots: [...spots], badSpots: [...badSpots] });
        spots = [];
        badSpots = [];
        drawStrikeZone();
        updateStats();
    ...
    document.getElementById('multiGameBtn').addEventListener('click', showMultiGameStats);
...
function undoLastAction() {
    ...
    if (lastAction.type === 'strike') {
        spots.pop();
        const strikesInput = document.getElementById('goodBalls');
        strikesInput.value = Math.max(0, parseInt(strikesInput.value || 0) - 1);
    } else if (lastAction.type === 'ball') {
        badSpots.pop();
    } else if (lastAction.type === 'clear') {
        spots = lastAction.spots;
        badSpots = lastAction.badSpots;
    }
...
// ===== Multi-Game Stats =====
function showMultiGameStats() {
    const history = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
    if (history.length === 0) {
        alert('No training records');
        return;
    }
    ...
    container.innerHTML = `
        ...
        <div id="multiSelectList" style="max-height: 300px; overflow-y: auto; margin: 15px 0;">
            ${history.map(record => `
                <div style="display: flex; align-items: center; padding: 8px; border-bottom: 1px solid #eee; cursor: pointer;" 
                     onclick="toggleMultiSelect(${record.id}, this)">
                    <input type="checkbox" id="check_${record.id}" style="margin-right: 10px;" />
                    <div style="flex: 1;">
                        <div style="font-weight: bold;">${record.trainingName}</div>
                        <div style="font-size: 12px; color: #666;">${record.trainingDate} | S: ${record.goodBalls} B: ${record.badBalls} | K: ${record.strikeouts}</div>
                    </div>
                </div>
            `).join('')}
        </div>
        <div style="text-align: center;">
            <button class="btn btn-primary" onclick="calculateMultiStats()">Calculate Combined Stats</button>
            <button class="btn btn-secondary" onclick="closeHistory()">Cancel</button>
        </div>
        <div id="multiStatsResult" style="margin-top: 15px;"></div>
    `;
...
function toggleMultiSelect(id, element) {
    ...
    if (checkbox.checked) {
        if (!selectedGameIds.includes(id)) selectedGameIds.push(id);
        element.style.background = 'rgba(231, 76, 60, 0.1)';
    } else {
        selectedGameIds = selectedGameIds.filter(gid => gid !== id);
        element.style.background = 'transparent';
    }
}

function calculateMultiStats() {
    ...
    const history = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
    const selectedGames = history.filter(r => selectedGameIds.includes(r.id));
    
    let totalStrikes = 0, totalBalls = 0, totalStrikeouts = 0, totalWalks = 0, totalHits = 0, totalRuns = 0;
    let totalSpots = 0, totalBadSpots = 0;
    
    selectedGames.forEach(g => {
        totalStrikes += g.goodBalls || 0;
        totalBalls += g.badBalls || 0;
        totalStrikeouts += g.strikeouts || 0;
        totalWalks += g.walks || 0;
        totalHits += g.hits || 0;
        totalRuns += g.runs || 0;
        totalSpots += (g.spots || []).length;
        totalBadSpots += (g.badSpots || []).length;
    });
    
    const totalPitches = totalStrikes + totalBalls;
    const strikeRate = totalPitches > 0 ? ((totalStrikes / totalPitches) * 100).toFixed(1) : 0;
    
    document.getElementById('multiStatsResult').innerHTML = `
        <div style="border: 2px solid #27ae60; padding: 15px; border-radius: 8px;">
            <h4 style="color: #27ae60; margin-bottom: 10px;">Combined Stats (${selectedGames.length} Games)</h4>
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <tr><td style="padding: 6px;">Games</td><td style="padding: 6px; text-align: right; font-weight: bold;">${selectedGames.length}</td></tr>
                <tr><td style="padding: 6px;">Total Strikes</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalStrikes}</td></tr>
                <tr><td style="padding: 6px;">Total Balls</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalBalls}</td></tr>
                <tr><td style="padding: 6px;">Total Pitches</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalPitches}</td></tr>
                <tr><td style="padding: 6px;">Strike Rate</td><td style="padding: 6px; text-align: right; font-weight: bold; color: #27ae60;">${strikeRate}%</td></tr>
                <tr><td style="padding: 6px;">Strikeouts</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalStrikeouts}</td></tr>
                <tr><td style="padding: 6px;">Walks</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalWalks}</td></tr>
                <tr><td style="padding: 6px;">Hits</td><td style="padding: 6px; text-align: right; font-weight: bold;">${totalHits}</td></tr>
                <tr><td style="padding: 6px; color: #e74c3c;">Runs</td><td style="padding: 6px; text-align: right; font-weight: bold; color: #e74c3c;">${totalRuns}</td></tr>
            </table>
            <div style="margin-top: 10px; font-size: 12px; color: #666;">
                <span style="color: #27ae60;">●</span> Strike marks: ${totalSpots}
                <span style="color: #e67e22; margin-left: 10px;">●</span> Ball marks: ${totalBadSpots}
            </div>
        </div>
    `;
}
